...
import com.siso.user.infrastructure.oauth2.*;
...
// OAuth2UserInfoFactory static mock
        try (MockedStatic<OAuth2UserInfoFactory> mockedFactory = mockStatic(OAuth2UserInfoFactory.class)) {

            mockedFactory.when(() -> OAuth2UserInfoFactory.getOAuth2UserInfo(TEST_PROVIDER, attributes)).thenReturn(userInfo);
            ...
            assertThat(result.getRegistrationStatus()).isEqualTo(RegistrationStatus.REGISTER);
    ...
    void loginWithProvider_shouldSaveRefreshTokenToDatabase() {
        ...
        when(oAuth2UserInfo.getEmail()).thenReturn(TEST_EMAIL);
        when(oAuth2UserInfo.getPhoneNumber()).thenReturn(TEST_PHONE);
        ...
        try (MockedStatic<OAuth2UserInfoFactory> mockedFactory = Mockito.mockStatic(OAuth2UserInfoFactory.class)) {
            ...
            mockedFactory.when(() -> OAuth2UserInfoFactory.getOAuth2UserInfo(TEST_PROVIDER, attributes)).thenReturn(oAuth2UserInfo);
            ...
            verify(userRepository).save(argThat(savedUser ->
                    TEST_JWT_REFRESH.equals(savedUser.getRefreshToken())
            ));
        }
    }
