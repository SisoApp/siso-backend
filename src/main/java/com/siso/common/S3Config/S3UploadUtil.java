...
//    @Value("${aws.s3.bucket}") // 로컬용
    ...
    public void putObject(String key, MultipartFile file, String contentType) {
        try {
            ...
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "AWS 자격 증명 오류로 파일을 업로드할 수 없습니다.");
        } catch (IOException ioe) {
            log.error("S3 업로드 실패 - 파일 읽기 오류 - key: {}, message: {}", key, ioe.getMessage(), ioe);
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "파일을 읽는 데 실패했습니다.");
        } catch (Exception e) {
            log.error("S3 업로드 실패 - 기타 오류 - key: {}, message: {}", key, e.getMessage(), e);
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "파일 업로드 중 알 수 없는 오류가 발생했습니다.");
        }
    }
